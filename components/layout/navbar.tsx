...
import {
  Menu, X, Sun, Moon, Cpu, ChevronDown,
  User as UserIcon, LogOut, LayoutDashboard, Shield, Crown, BookOpen, GraduationCap,
} from 'lucide-react'
...
export default function Navbar() {
  ...
  const dashHref = user ? defaultPortal(user.role) : '/login'
  const RoleIcon = user ? (roleIcon[user.role] ?? UserIcon) : UserIcon
        ...
        <div className="flex items-center gap-1.5">
          ...
          {mounted && status === 'loading' ? (
            ...
            <div className="relative">
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-56 glass rounded-lg shadow-2xl py-1 z-50 overflow-hidden">
                  <div className="px-3 py-2.5 border-b border-border">
                    <p className="text-sm font-semibold truncate">{user.name}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                      <RoleIcon className="w-3 h-3 text-primary" />
                      <p className="text-[10px] font-mono text-primary uppercase tracking-wider">{user.role.replace('_', ' ')}</p>
                    </div>
                  </div>
                  <Link href={dashHref} className="flex items-center gap-2.5 px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-muted" onClick={() => setUserMenuOpen(false)}>
                    <LayoutDashboard className="w-4 h-4" /> Dashboard
                  </Link>
                  {user.role === 'user' && (
                    <Link href="/member/profile" className="flex items-center gap-2.5 px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-muted" onClick={() => setUserMenuOpen(false)}>
            ...
            <Link href="/login"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono border border-primary/40 bg-primary/5 text-primary rounded-md hover:bg-primary/15 transition-colors">
              Sign In
            </Link>
          ) : null}
        ...
        <div className="lg:hidden bg-card border-b border-border">
          <ul className="container mx-auto px-4 py-3 flex flex-col gap-1">
            ...
            <li className="border-t border-border mt-2 pt-2">
              {user ? (
                <Link href={dashHref} className="flex items-center gap-2 px-3 py-2.5 text-sm text-primary" onClick={() => setMobileOpen(false)}>
                  <LayoutDashboard className="w-4 h-4" /> Dashboard
                </Link>
              ) : (
                <Link href="/login" className="flex items-center gap-2 px-3 py-2.5 text-sm text-primary" onClick={() => setMobileOpen(false)}>
                  Sign In →
                </Link>
              )}
            </li>
