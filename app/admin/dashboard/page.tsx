...
import {
  ...
  Send, UserCheck, GraduationCap, ClipboardList
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const apprenticeData = useApiData<{ apprentices: any[] }>('/api/apprentices', { apprentices: [] })
  const apprentices    = apprenticeData.apprentices ?? []
  ...
  const summaryCards = [
    ...
    { label: 'My Apprentices',     value: apprentices.length,   icon: GraduationCap, sub: 'members & creators you mentor',  color: 'text-sky-400',     href: '/admin/attendance' },
  ]
  ...
  const quickLinks = [
    ...
    { label: 'Attendance & Hours', href: '/admin/attendance', icon: ClipboardList, desc: `Mark attendance for ${apprentices.length} apprentice${apprentices.length !== 1 ? 's' : ''}` },
